  ...
  return (
    <div className="mx-auto max-w-container-max-width px-4 py-6 md:px-margin-lg md:py-8">
        ...
        <div className="relative grid gap-8 lg:grid-cols-[1.3fr,0.7fr] lg:items-center">
          <div className="space-y-5">
            ...
            <div className="space-y-4">
              <h1 className="max-w-3xl text-4xl font-extrabold leading-tight tracking-[-0.02em] text-primary md:text-[56px]">{title}</h1>
              <p className="max-w-2xl font-[var(--font-body)] text-base leading-8 text-on-surface-variant md:text-lg">{description}</p>
            </div>
            <div className="flex flex-wrap gap-4">
              {actions.map((action, index) => (
                <Link
                  ...
                  className={
                    index === 0
                      ? 'inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-primary-container to-secondary-container px-6 py-4 text-sm font-bold text-on-primary shadow-[0_10px_24px_rgba(0,218,243,0.24)] transition-all hover:-translate-y-0.5'
                      : 'inline-flex items-center gap-2 rounded-xl border border-outline-variant px-6 py-4 text-sm font-bold text-on-surface transition-all hover:bg-surface-variant/20'
                  }
                >
                  {action.label}
                  <ArrowRight size={16} />
          ...
          <div className="glass-panel isometric-card rounded-[1.75rem] p-6 md:p-8">
      ...
      <section className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {stats.map((stat) => (
          <div key={stat.label} className="glass-panel glass-panel-hover rounded-[1.5rem] p-6">
            <p className="text-[10px] font-black uppercase tracking-[0.22em] text-on-surface-variant">{stat.label}</p>
            <p className={`mt-3 text-3xl font-extrabold ${toneClasses[stat.tone || 'neutral']}`}>{stat.value}</p>
          </div>
        ))}
      </section>
      ...
      <section className="mt-8 grid gap-6 lg:grid-cols-3">
        {cards.map((card) => {
          ...
          return (
            <article key={card.title} className="glass-panel glass-panel-hover isometric-card rounded-[1.75rem] p-6">
              <div className="mb-6 flex h-14 w-14 items-center justify-center rounded-2xl bg-primary-fixed-dim/10 text-primary-fixed-dim">
                <CardIcon size={24} />
              </div>
              <h2 className="text-2xl font-bold text-on-surface">{card.title}</h2>
